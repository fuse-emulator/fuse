...
creator_uses_legacy_z80( const libspectrum_creator *creator )
{
  ...
  if( !program || strcmp( program, "Spectaculator" ) ) return 0;

  major = libspectrum_creator_major( creator );
  minor = libspectrum_creator_minor( creator );

  /* Spectaculator 6.25 introduced selectable CPU behaviour. Earlier
     versions used a single, simpler Z80 model. Build 550 was the first
     public 6.25 release; builds with major version 62 before that are 6.20. */
...
start_playback( libspectrum_rzx *from_rzx )
{
  ...
  rzx_cmos_forced =
    creator_uses_legacy_z80( libspectrum_rzx_creator( from_rzx ) ) &&
